...
#### 收斂圖:
![image](https://user-images.githubusercontent.com/68068287/157604945-140683a0-95f2-4be7-9752-be4182cfa0fc.png)
...
#### 收斂圖:
![image](https://user-images.githubusercontent.com/68068287/157605030-16312f90-c2cc-4c0d-a145-84d7693e371c.png)

## 結語
#### 本專案使用3種演算法解決背包問題，其中我認為動態規劃演算法較能達到最佳解。爬山演算法及模擬退火演算法是透過設定最大迭代次數期望在次數內找到最佳解，但是基本上每一次執行這兩個演算法不見得能在最大迭代次數內找到最佳解，因為初始解是隨機生成，且每一次迭代也都是使用隨機的方式產生當前解，在所有情況皆為隨機的情形下，就無法保證能在最大迭代次數內找到最佳解，本專案使用500作為最大迭代次數，增加迭代次數不見得能提高找到最佳解的可能性，因為每一次迭代產生的解不會被記錄起來，舉例來說，上一次迭代的解不是目前最佳解所以沒被保存，下下次迭代或許可能又產生同樣的解，因為沒有保存非最佳解的機制，有機率使得同一組非最佳解會重複出現，因此我認為就算設再大的迭代次數也不見得找得到全局最佳解，我認為動態規劃演算法較適合用來找出背包問題的最佳解。
